...
def get_squad(venue_name, selected_players):
    """Filter dataset for selected venue and players, assign zero stats for missing players"""
    venue_df = df[df['Venue'] == venue_name].copy()
    squad = venue_df[venue_df["Player"].isin(selected_players)].copy()
    
    found_players = set(squad["Player"])
    missing_players = set(selected_players) - found_players

    # Add dummy players with zero stats if not found
    if missing_players:
        dummy_rows = []
        for player in missing_players:
            dummy_rows.append({
                "Player": player,
                "Role": "Batter",  # Default role
        ...
        squad = pd.concat([squad, dummy_df], ignore_index=True)
...
def crossover(parent1, parent2, squad):
    ...
    if len(set(child)) < 11:
        # Fill with random players not in the team
